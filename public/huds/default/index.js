...
function updateHUD(data) {
  ...
  const left = Object.values(data.allplayers).filter(player => player.team === 'CT');
  const right = Object.values(data.allplayers).filter(player => player.team === 'T');
  const teams_players = {
  ...
  window.allplayers = data.allplayers;
    ...
    const round_now = data.round_now;
  ...
  updateTopPanel(left, right, data.map, data.phase_countdowns, data.bomb, data.match, data.round_now);
  updateRoundNow(data.round, data.map);
  updateRoundState(data.phase_countdowns, data.round, data.map, data.previously, data.bomb, data.players, left, right, teams);
  fillObserved(data.player);
  updatePlayers(data.allplayers, data.players, data.observed, data.phase, data.previously, teams, teams_players, data.map);
  updateTeamValues(left, right, data.map);;
  countNades(left, right);
  updatePlayerNames(data.allplayers);
  updateRadar(data.allplayers, data.map, data, data.player, data.bomb, data.player, data.grenades)
  freezetime = data.round.phase == "freezetime";
  last_round = data.round_now;
}



function updateTopPanel(left, right, map, phase_countdowns, bomb, match, round_now) {
  ...
  //var matchup = match?.format || 'bo1';
    ...
    var round = map.round;
        ...
        if (!map || !left || !right) {
          ...
          $('#left_team #best_of .block1, #left_team #best_of .block2, #left_team #best_of .block3, #right_team #best_of .block1, #right_team #best_of .block2, #right_team #best_of .block3').css('background-color', '');
        ...
        $("#left_team #main, .live_left, #left_team #score, #right_team #main, .live_right, #right_team #score").css("color", COLOR_WHITE);
...
// Создаем полосы для best_of
const matchFormat = match?.format || 'bo1'; // Получаем формат матча
const scoreTeamCT = map.team_ct.matches_won_this_series || 0; // Счет CT команды
const scoreTeamT = map.team_t.matches_won_this_series || 0; // Счет T команды
...
$('#best_of .block1, #best_of .block2, #best_of .block3, #best_of .block4, #best_of .block5').css('background-color', '');

// Устанавливаем цвет основной полосы
$('#left_team #best_of .bar').css('background-color', teams.left.side === 'ct' ? COLOR_NEW_CT : COLOR_NEW_T);
$('#right_team #best_of .bar').css('background-color', teams.right.side === 'ct' ? COLOR_NEW_CT : COLOR_NEW_T);

      if (matchFormat === 'bo3') {
        if (scoreTeamCT == 0) {
          $('#left_team #best_of .block4').css('opacity', '1');
          $('#left_team #best_of .block5').css('opacity', '1');
        }

        if (scoreTeamT == 0) {
          $('#right_team #best_of .block4').css('opacity', '1');
          $('#right_team #best_of .block5').css('opacity', '1');
        }

          // Для левой команды
          if (scoreTeamCT == 1) {
              $('#left_team #best_of .block4').css('background-color', COLOR_WHITE).css('opacity', '1');
              $('#left_team #best_of .block5').css('opacity', '1');
          }
          if (scoreTeamCT == 2) {
              $('#left_team #best_of .block5').css('background-color', COLOR_WHITE).css('opacity', '1');
              $('#left_team #best_of .block4').css('background-color', COLOR_WHITE).css('opacity', '1');
          }
          // Для правой команды
          if (scoreTeamT == 1) {
              $('#right_team #best_of .block4').css('background-color', COLOR_WHITE).css('opacity', '1');
              $('#right_team #best_of .block5').css('opacity', '1');
          }
          if (scoreTeamT == 2) {
              $('#right_team #best_of .block5').css('background-color', COLOR_WHITE).css('opacity', '1');
              $('#right_team #best_of .block4').css('background-color', COLOR_WHITE).css('opacity', '1');
          }
      } else if (matchFormat === 'bo5') {

          if (scoreTeamCT == 0) {
            $('#left_team #best_of .block1').css('opacity', '1');
            $('#left_team #best_of .block2').css('opacity', '1');
            $('#left_team #best_of .block3').css('opacity', '1');
          }

          if (scoreTeamT == 0) {
            $('#right_team #best_of .block1').css('opacity', '1');
            $('#right_team #best_of .block2').css('opacity', '1');
            $('#right_team #best_of .block3').css('opacity', '1');
          }
            // Для левой команды
            if (scoreTeamCT == 1) {
                $('#left_team #best_of .block1').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#left_team #best_of .block2').css('opacity', '1');
                $('#left_team #best_of .block3')('opacity', '1');
            }
              if (scoreTeamCT == 2) {
                $('#left_team #best_of .block1').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#left_team #best_of .block2').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#left_team #best_of .block3').css('opacity', '1');
            }
            if (scoreTeamCT == 3) {
                $('#left_team #best_of .block1').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#left_team #best_of .block2').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#left_team #best_of .block3').css('background-color', COLOR_WHITE).css('opacity', '1');
            }
            // Для правой команды
            if (scoreTeamT == 1) {
                $('#right_team #best_of .block1').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#right_team #best_of .block2').css('opacity', '1');
                $('#right_team #best_of .block3').css('opacity', '1');
            }
            if (scoreTeamT == 2) {
                $('#right_team #best_of .block1').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#right_team #best_of .block2').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#right_team #best_of .block3').css('opacity', '1');
            }
            if (scoreTeamT == 3) {
                $('#right_team #best_of .block3').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#right_team #best_of .block2').css('background-color', COLOR_WHITE).css('opacity', '1');
                $('#right_team #best_of .block1').css('background-color', COLOR_WHITE).css('opacity', '1');
            }
    } else {
...
function updateRoundState(phase_countdowns, round, map, previously, bomb, players, left, right, teams) {
...
function updateStateOver(phase_countdowns, round, previously, teams, left_alive, right_alive) {
      ...
      if (bomb != null) {
        if (bomb.state == "defusing") {
          ...
          showAlertSlide(
            defusing_side,
            COLOR_NEW_CT,
            defuser.name + " is defusing the bomb"
          );
        }
      }
    }
...
function updateStatePaused(phase, type, previously, map) {
  ...
  let leftTeamName = map.team_ct.name ? map.team_ct.name.toUpperCase() : "UNKNOWN";
  let rightTeamName = map.team_t.name ? map.team_t.name.toUpperCase() : "UNKNOWN";
  ...
  if (type == "paused") {
    ...
    $("#alert_middle #alert_text_middle")
      .text(
        map.team_t.side == "t"
          ? rightTeamName + " TIMEOUT"
          : leftTeamName + " TIMEOUT"
      )
      .css("color", COLOR_NEW_T);
    showAlertSlide(
      "#left_team",
...
/*function updateObserved(players) {
  console.log(players);
  if (players && players.steamid != 1) {
    $("#observed").css("opacity", 1);
    fillObserved(players);
  } else {
...
function fillObserved(player) {
  ...
  if (player.avatar) {
    ...
    const defaultLogo = player.team === "CT" ? "/images/logo_ct_default.png" : "/images/logo_t_default.png";
  ...
  $("#obs_assists_text").text(player.match_stats ? player.match_stats.assists : 0);
  $("#obs_deaths_d").css("color", team_color);
  $("#obs_deaths_text").text(player.match_stats ? player.match_stats.deaths : 0);
...
function updatePlayers(allplayers, players, observed, phase, previously, teams, teams_players, map) {
  if (teams_players) {
    fillPlayers(allplayers, teams, teams_players, observed, phase, previously, map);
  }
}

function fillPlayers(allplayers, teams, teams_players, observed, phase, previously, map) {
  if (teams_players?.left?.players) {
    for (var i = 0; i < 5; i++) {
      if (i >= teams_players.left.players.length) {
        ...
        if (player) {  // Проверяем, что игрок существует
          fillPlayer(
            allplayers,
            teams,
            player,
...
function fillPlayer(allplayers, teams, player, nr, side, observed, phase, previously, map) {
  ...
  let alt_health_color = stats.health <= 20 ? COLOR_RED : team == "CT" ? COLOR_NEW_CT : COLOR_NEW_T;
  ...
  $top
    ...
    if (disp_player_avatars) {
      if (player.hasOwnProperty("avatar") && player.avatar) {
        ...
        // Если логотип команды не найден, путь пустой или некорректный — дефолт
        if (!_img || _img === "/uploads/" || _img.includes("undefined") || _img.includes("null")) {
          _img = team === "CT" ? "/images/logo_ct_default.png" : "/images/logo_t_default.png";
        }
        $player.find("#player_image")
          .attr("src", _img)
          .addClass(dead ? "dead" : "");
...
function updatePlayerNames(allplayers) {
  Object.values(allplayers).forEach(player => {
    ...
    const playerAliasElement = document.querySelector(`#player_alias_text_${slot}`);
...
function resetBomb(teams) {
  clearInterval(bomb_timer);
  // Всегда сбрасываем состояние бомбы
  $("#timers #bomb_bar").css("opacity", 0);
  $("#timers #defuse_bar").css("opacity", 0);
...
function updateTeamValues(left, right, map) { 
  ...
  const leftTeamMoney = leftArray.reduce((sum, player) => sum + (player.state?.money || 0), 0);
  const rightTeamMoney = rightArray.reduce((sum, player) => sum + (player.state?.money || 0), 0);
  ...
  const leftEquipValue = leftArray.reduce((sum, player) => sum + (player.state?.equip_value || 0), 0);
  const rightEquipValue = rightArray.reduce((sum, player) => sum + (player.state?.equip_value || 0), 0);
  ...
  function initializeRadar() {
    ...
    window.radar.loadMapData = function(mapName) {
      ...
      fetch(metaPath)
        ...
        .catch(error => console.warn('Не удалось загрузить настройки карты:', error));
    };
          ...
          if (player.weapons) {
            ...
            if (lastAmmo !== undefined && currentAmmo < lastAmmo) {
              ...
              if (!isNaN(x) && !isNaN(y)) {
                ...
                fireElement.style.transform = `translate(calc(-50% + ${translateX}px), calc(-50% + ${translateY}px)) rotate(${angle + 90}deg)`;
              }
            }
...
// ... existing code ...
    ...
    window.radar.positionToPixels = function(position) {
      ...
      const lowerLevelRange = this.splits?.[0]?.zRange || { min: -Infinity, max: Infinity };
      const upperLevelRange = this.zRange || { min: -Infinity, max: Infinity };
      
      // Определяем, на каком этаже находится объект
      const isLowerLevel = posZ >= lowerLevelRange.min && posZ <= lowerLevelRange.max;
      const isUpperLevel = posZ >= upperLevelRange.min && posZ <= upperLevelRange.max;
      ...
      let x = ((posX + this.offset.x) / (1024 * this.resolution)) * containerWidth * 1.02;
      let y = containerHeight - ((posY + this.offset.y) / (1024 * this.resolution)) * containerHeight * 1.02;
  ...
  function updateRadar(allplayers, map, data, observed, bomb, player, grenades) {
    ...
    if (map?.name && (!window.radar.currentMap || window.radar.currentMap !== map.name)) {
    ...
    if (allplayers && window.radar) {
      ...
      if (!window.radar.domCache.radarContainer) {
        window.radar.domCache.radarContainer = document.getElementById('radar-players');
      }
      ...
      if (window.radar.frameCounter % 2 === 0) {
        ...
        Object.entries(allplayers).forEach(([steamid, player]) => {
          ...
          if (playerElement) {
            ...
            if (hasBomb) {
              ...
              playerElement.style.backgroundColor = player.team.toLowerCase() === 'ct' ? COLOR_NEW_CT : COLOR_NEW_T;
            }
            
            // Обработка слотов
            let slotIdentifier;
            if (typeof player.observer_slot === 'number') {
              slotIdentifier = player.observer_slot === 9 ? 0 : player.observer_slot + 1;
            } else {
              slotIdentifier = 'N/A';
            }
            ...
            playerElement.classList.toggle('active_player_map', steamid === window.activePlayerSteamId);
          }
          
          // Проверяем, изменилась ли позиция игрока
          const position = player.position ? player.position.split(', ').map(Number) : null;
          ...
          if (position && (!player.lastPosition || 
            ...
            player.lastPosition = { x: position[0], y: position[1], z: position[2] };
          }
        });
        ...
        if (window.radar.frameCounter % 10 === 0 && radarContainer) {
          const currentPlayerElements = Array.from(radarContainer.querySelectorAll('[id^="player-"]'));
...
function clearAllRadarElements() {
  ...
  document.querySelectorAll('.grenade, .inferno-flame, .inferno-container, .flame-point').forEach(el => el.remove());
  
  // Очищаем бомбу
  document.querySelectorAll('#bomb-container').forEach(el => el.remove());
  
  // Очищаем контейнеры гранат
  const grenadeContainers = ['#smokes', '#flashbangs', '#hegrenades', '#infernos'];
...
function createPlayerElement(player, steamid) {
  ...
  playerElement.style.backgroundColor = player.team === 'CT' ? COLOR_NEW_CT : COLOR_NEW_T;
 ...
 const hasBomb = Object.values(player.weapons || {}).some(weapon => weapon.type === 'C4');
 if (hasBomb) {
     playerElement.style.backgroundColor = PLAYER_ORANGE;
 }
...
function updatePlayerElement(element, player) {
  ...
  if (hasBomb) {
    ...
    element.style.backgroundColor = player.team.toLowerCase() === 'ct' ? COLOR_NEW_CT : COLOR_NEW_T;
  }
...
function updateGrenades(grenades, allplayers) {
  ...
  if (!grenades || Object.keys(grenades).length === 0 || !allplayers || !window.radar?.splits) return;
  ...
  try {
    ...
    const firebombsContainer = document.getElementById('firebombs') || infernosContainer; // Используем отдельный контейнер или fallback

    if (!smokesContainer || !flashesContainer || !heContainer || !infernosContainer) return;
    ...
    grenadeKeys.slice(0, MAX_GRENADES_PER_FRAME).forEach(id => {
      ...
      if (grenade.type === 'smoke' || grenade.type === 'flashbang' || 
          grenade.type === 'frag' || grenade.type === 'firebomb' ||
        ...
        const grenadeType = isSmoke ? 'smoke' : 
                          (isFlash ? 'flash' : 
                          (isFrag ? 'frag' : 
                          (isInferno ? 'inferno' : 
                          (isFirebomb ? 'firebomb' : ''))));
        ...
        const container = isSmoke ? smokesContainer : 
                        (isFlash ? flashesContainer : 
                        (isFrag ? heContainer : 
                        (isFirebomb ? firebombsContainer : infernosContainer)));
...
if (isInferno) {
  ...
  if (grenade.flames) {
    ...
    flameEntries.slice(0, MAX_FLAMES_PER_FRAME).forEach(([flameId, position]) => {
      existingFlames.add(flameId);
      let flameElement = document.getElementById(`${grenadeId}_${flameId}`);
      ...
      const flamePos = typeof position === 'string' ? 
        ...
        [position.x, position.y, position.z];
              ...
              flameElement.setAttribute('data-level', isLowerLevel ? 'lower' : 'upper');
            });

            // Удаляем только те flames, которых больше нет в данных
            if (lifetime >= 22) {
              grenadeElement.querySelectorAll('.flame-point').forEach(flame => {
...
if (!grenadeElement) {
  ...
  if (isFirebomb) {
    ...
    const teamClass = `util_${ownerTeam === 'T' ? 'molotov' : 'incgrenade'}_${ownerTeam || 'default'}`;
    grenadeElement.classList.add(teamClass);
    
    const imgElement = document.createElement('img');
    imgElement.className = 'firebomb-icon';
    imgElement.src = `/images/maps/element/grenades/weapon_${ownerTeam === 'T' ? 'molotov' : 'incgrenade'}_${ownerTeam || 'default'}.webp`;
    
    
    grenadeElement.appendChild(imgElement);
  } else {
    grenadeElement.className = isSmoke ? 'smoke-grenade' : 
                              (isFlash ? 'flash-grenade' : 'hegrenades-grenade');
  
    const ownerTeam = playerTeams[grenade.owner];
    const teamClass = `util_${isSmoke ? 'smokegrenade' : (isFlash ? 'flashbang' : 'hegrenade')}_${ownerTeam || 'default'}`;
    grenadeElement.classList.add(teamClass);
    
    const imgElement = document.createElement('img');
    imgElement.className = isSmoke ? 'smoke-icon' : 
                        (isFlash ? 'flash-icon' : 'hegrenades-icon');
    
    // Исправляем путь к изображению
    const grenadeType = isSmoke ? 'smokegrenade' : (isFlash ? 'flashbang' : 'hegrenade');
    imgElement.src = `/images/maps/element/grenades/weapon_${grenadeType}${ownerTeam ? '_' + ownerTeam : ''}.webp`;
  ...
  container.appendChild(grenadeElement);
          ...
          grenadeElement.setAttribute('data-level', isLowerLevel ? 'lower' : 'upper');
          grenadeElement.style.transition = 'left 0.1s linear, bottom 0.1s linear';
          ...
          if (isSmoke) {
            ...
            if (lifetime >= FIREBOMB_MAX_LIFETIME && !grenadeElement.dataset.exploded) {
              grenadeElement.dataset.exploded = 'true';
              grenadeElement.style.transition = 'none';
              
              const iconElement = grenadeElement.querySelector('.firebomb-icon');
              if (iconElement) {
                iconElement.remove();
              }
            ...
            if (lifetime >= FLASH_MAX_LIFETIME && !grenadeElement.dataset.exploded) {
              grenadeElement.dataset.exploded = 'true';
              grenadeElement.style.transition = 'none';
            ...
            if (lifetime >= FRAG_MAX_LIFETIME) {
              if (grenadeElement && !grenadeElement.dataset.exploded && !grenadeElement.dataset.animationStarted) {
                grenadeElement.dataset.exploded = 'true';
                grenadeElement.dataset.animationStarted = 'true';
                grenadeElement.style.transition = 'none';
                
                const iconElement = grenadeElement.querySelector('.hegrenades-icon');
                if (iconElement) {
                  iconElement.remove();
                }
    ...
    if (infernosContainer) {
      infernosContainer.querySelectorAll('.inferno-container').forEach(element => {
    ...
    if (Math.random() < 0.2) {
      ...
      if (firebombsContainer) {
        firebombsContainer.querySelectorAll('.firebomb-grenade').forEach(element => {
...
function clearAllGrenades() {
  const containers = ['#smokes', '#flashbangs', '#hegrenades', '#infernos', '#firebombs'];

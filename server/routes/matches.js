...
router.post("/:id/update", async (req, res) => {
  const db = req.app.locals.db;
  const matchId = req.params.id;
  const { format, maps, team1_id, team2_id } = req.body;
  ...
  db.serialize(() => {
    ...
    try {
      ...
      if (format || team1_id || team2_id) {
        ...
        console.log(
          `Обновлены данные матча ${matchId}: формат=${format}, team1_id=${team1_id}, team2_id=${team2_id}`
        );
      }
